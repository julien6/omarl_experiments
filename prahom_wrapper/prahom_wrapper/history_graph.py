...
if __name__ == '__main__':
    ...
    def rdm_occ_card(card: cardinality) -> int:
        min_card = card[0]
    ...
    def sample(pattern_string: history_pattern):

        def sample_aux(history_tuples):

            if history_graph.is_only_labels(history_tuples):
                seq, card = history_tuples
                seq = ["o" if label == "#any" else label for label in seq]
                return ",".join(rdm_occ_card(card) * seq)
                ...
                return ",".join([generated_seq] * rdm_occ_card(card))

        return sample_aux(history_graph.parse_into_tuple(pattern_string)).replace(",,", ",")

    # def match2(pattern_string, string):

    #     def match_aux(history_tuples, tmp_string):

    #         if history_graph.is_only_labels(history_tuples):
    #             seq, card = history_tuples

    #             if len(tmp_string) == 0:
    #                 return False, tmp_string, None
    ...
    #             pattern = f"^(?:{sub_string}[,]{{0,1}}){{{card_min},{card_max}}}(?!{sub_string}[,]{{0,1}})"
    #             # print(pattern)
    #             # print(tmp_string)

    #             matches = re.match(pattern, tmp_string)
    #             if matches:
    #                 matched = matches.group()
    #                 tmp_string = tmp_string[len(matched):]
    #                 if len(tmp_string) > 0 and tmp_string[0] == ",":
    #                     tmp_string = tmp_string[1:]
    #                 return True, tmp_string, None
    ...
    #             while (still_match and len(tmp_string) > 0):
    #                 for sub_seq in seq:
    #                     match, tmp_string, _ = match_aux(
    #                         sub_seq, tmp_string)
    #                     still_match = still_match and match
    #                     if not still_match:
    #                         next_expected_seq = sub_seq
    #                         break
    #                 card_counter += 1
    #                 if card_counter >= card_max:
    #                     next_expected_seq = sub_seq
    #                     break
    #             still_match = still_match and card_min <= card_counter and card_counter <= card_max
    #             return still_match, tmp_string, next_expected_seq

    #     tmp_string = copy.copy(string)
    #     match, rest, next_expected_seq = match_aux(
    #         history_graph.parse_into_tuple(pattern_string), tmp_string)
    #     matched = string[0:(len(string)-len(rest))]
    #     if matched[-1] == ",":
    #         matched = matched[:-1]

    #     return match, matched, next_expected_seq

    def match(pattern_string, string):
        ...
        pattern, overall_length, last_sequence = match_aux(
            history_graph.parse_into_tuple(pattern_string), 0, 0, -1)
        ...
        for i in range(1, overall_length + 1):
            pattern, length, last_sequence = match_aux(
                history_graph.parse_into_tuple(pattern_string), 0, 0, i)

            old_pattern = copy.copy(pattern)
        ...
        if next_expected_sequence:
            ...
            next_expected_sequence = (next_expected_sequence[0], cardinality(next_expected_sequence[1][0],next_expected_sequence[1][1]))

        return is_matched, matched, float(length / overall_length), next_expected_sequence

    history = "[0,1,2,3,[#any](0,*),4,5](1,1)"

    s1 = "0,1,2,3,78,89,4,5"

    print(match(history, s1))

    # print(match(history, s1))

    # rules = {
    #     "[o1,a2,[#any](0,*),o10](1,1)": ["a1", "a2", "a3"]
    # }
    ...
    # - observable_policy_constraint OK
    # - constrained_policy
